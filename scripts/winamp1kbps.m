...
System.onScriptLoaded(){
	...
	khzbitrateText = player.findObject("khzbitrate");
	...
	if(getStatus() == 0){
		khzbitrateText.setText("0kHz, 0kbps");
	}
}
...
getSonginfo(String SongInfoString) {
	...
	if(tkn != "") {bitratestring = (tkn);}
	if(bitrateint == 0 || bitrateint == -1) {bitratestring = ("0");}
	...
	if(tkn != "") {frequencystring = (tkn);}
	if(freqint == 0 || freqint == -1 ) {frequencystring = ("0");}

	khzbitrateText.setText(frequencystring+"kHz, "+bitratestring+"kbps");
